...
SerialPortManager::SerialPortManager(QObject *parent) : QObject(parent), serialPort(nullptr), m_serialWorkerThread(new QThread(nullptr)), serialTimer(new QTimer(nullptr)),
    ...
    connect(m_protocol.get(), &SerialProtocol::getInfoReceived, this, [this](bool targetConnected, uint8_t indicators) {
        isTargetUsbConnected = targetConnected;
        ...
        QTimer::singleShot(4000, this, [this, currentPortName]() {
            ...
            if (serialPort && serialPort->setRequestToSend(false)) {
                ...
                QTimer::singleShot(500, this, [this, currentPortName]() {
                    ...
                    // Wait for device reboot to complete, then reconnect
                    QTimer::singleShot(1000, this, [this, currentPortName]() {
                        ...
                        QTimer::singleShot(2000, this, [this]() {
                            bool finalSuccess = ready;
                            qCDebug(log_core_serial) << "Factory reset reconnection completed, ready status:" << finalSuccess;
                            
                            emit factoryResetCompleted(finalSuccess);
                        });
                    });
                });
...
{
    if (m_watchdog) {
        return m_watchdog->getRetryAttemptCount();
    }
    return 0;
}
...
void SerialPortManager::resetErrorCounters()
{
    ...
    // Also reset watchdog counters (Phase 3)
    if (m_watchdog) {
